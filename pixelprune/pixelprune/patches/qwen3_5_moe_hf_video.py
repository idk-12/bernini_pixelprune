"""
HuggingFace Qwen3.5 MoE (原生 VL 模型) monkey-patch：在不动 modeling 源码的前提下注入 PatchSelector 选择器与 dedup 逻辑。
...
def _synchronize(accelerator: str) -> None:
    _accel_call(accelerator, "synchronize")
...
def _vision_forward(
    ...
    **kwargs: Any,
    ...
    """Patched Qwen3_5MoeVisionModel.forward：支持可选的 keep_indices 和 VIT_PRUNE_LAYER 中间层裁剪。"""
...
def _model_get_image_features(
    ...
    **kwargs: Any,
    ...
    if SHOULD_PROFILE:
        ...
        _synchronize(accelerator)
        _reset_peak_memory_stats(accelerator)
        mem_vit_before = _memory_allocated_gb(accelerator)
    ...
    if SHOULD_PROFILE:
        mem_vit_after = _memory_allocated_gb(accelerator)
        mem_vit_peak = _max_memory_allocated_gb(accelerator)
        _reset_peak_memory_stats(accelerator)
...
def _model_forward(
    ...
    **kwargs: Any,
    ...
    """Patched Qwen3_5MoeModel.forward (compatible with transformers 5.2.0)."""
...
def _cg_forward(
    ...
    **kwargs: Any,
    ...
    """Patched Qwen3_5MoeForConditionalGeneration.forward：在需要时计算 keep_indices 并传入 model。"""
    keep_indices = None
...
def apply_patches() -> None:
    ...
    if getattr(m.Qwen3_5MoeVisionModel, "__patch_selector_patched__", False):
        return

    _orig_vision_forward = m.Qwen3_5MoeVisionModel.forward
    m.Qwen3_5MoeVisionModel.forward = _vision_forward

    _orig_model_get_image_features = m.Qwen3_5MoeModel.get_image_features
    m.Qwen3_5MoeModel.get_image_features = _model_get_image_features

    _orig_model_forward = m.Qwen3_5MoeModel.forward
    m.Qwen3_5MoeModel.forward = _model_forward

    _orig_cg_forward = m.Qwen3_5MoeForConditionalGeneration.forward
    m.Qwen3_5MoeForConditionalGeneration.forward = _cg_forward

    m.Qwen3_5MoeVisionModel.__patch_selector_patched__ = True
    m.Qwen3_5MoeModel.__patch_selector_patched__ = True
    m.Qwen3_5MoeForConditionalGeneration.__patch_selector_patched__ = True
