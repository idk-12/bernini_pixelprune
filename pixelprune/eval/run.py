# ── PixelPrune: ensure VLMEvalKit's vlmeval is used as base ──────────────────
# Python prepends the script's directory (eval/) to sys.path, which would cause
# the incomplete eval/vlmeval package to be imported instead of VLMEvalKit's.
# We remove that implicit entry and insert VLMEvalKit explicitly first.
import sys as _sys, os as _os
_script_dir = _os.path.dirname(_os.path.abspath(__file__))
_repo_dir = _os.path.dirname(_script_dir)
_VLMEVAL_KIT = _os.path.join(_os.path.dirname(_repo_dir), 'VLMEvalKit')
if _script_dir in _sys.path:
    _sys.path.remove(_script_dir)
if _VLMEVAL_KIT not in _sys.path:
    _sys.path.insert(0, _VLMEVAL_KIT)
# ─────────────────────────────────────────────────────────────────────────────
...
# ── PixelPrune: inject timing-enabled Qwen3VLChat before config is evaluated ──
def _inject_pixelprune_models():
    ...
    # Must fully import vlmeval (VLMEvalKit) BEFORE loading our model so that
    # relative imports inside model.py (e.g. `from ..base import BaseModel`)
    # resolve against the already-initialized VLMEvalKit package tree.
    import vlmeval  # noqa: F401

    _q3vl = os.path.join(_eval_dir, 'vlmeval', 'vlm', 'qwen3_vl')
    if not os.path.isdir(_q3vl):
        return  # eval/vlmeval not present, skip

    def _load_as(mod_name, file_path, pkg):
    ...
    _load_as('vlmeval.vlm.qwen3_vl.prompt', os.path.join(_q3vl, 'prompt.py'), 'vlmeval.vlm.qwen3_vl')
    _model_mod = _load_as('vlmeval.vlm.qwen3_vl.model', os.path.join(_q3vl, 'model.py'), 'vlmeval.vlm.qwen3_vl')
    return _model_mod.Qwen3VLChat

_PP_Chat = _inject_pixelprune_models()
# ─────────────────────────────────────────────────────────────────────────────
...
if LOCAL_WORLD_SIZE > 1 and len(GPU_LIST):
    ...
    assert GPU_PER_PROC > 0 and NGPU % LOCAL_WORLD_SIZE == 0, (
        f'Cannot split {NGPU} visible devices evenly across {LOCAL_WORLD_SIZE} processes'
    )
    ...
    # Set both CUDA and Ascend visibility. The latter lets each rank use its
    # own model-parallel NPU group while WORLD_SIZE still shards the dataset.
    os.environ['CUDA_VISIBLE_DEVICES'] = VISIBLE_DEVICES
    os.environ['ASCEND_VISIBLE_DEVICES'] = VISIBLE_DEVICES
    os.environ['ASCEND_RT_VISIBLE_DEVICES'] = VISIBLE_DEVICES
    print(
        f'RANK: {RANK}, LOCAL_RANK: {LOCAL_RANK}, WORLD_SIZE: {WORLD_SIZE},'
        f'LOCAL_WORLD_SIZE: {LOCAL_WORLD_SIZE}, VISIBLE_DEVICES: {VISIBLE_DEVICES}'
    )
...
# # ── PixelPrune: register Qwen3-VL-HF and Qwen3.5-HF model configs ────────────
if _PP_Chat is not None:
    _mp = os.environ.get('model_path', 'Qwen/Qwen3-VL-2B-Instruct')
    _mnt = int(os.environ.get('max_new_tokens', 512))
    _common = dict(use_custom_prompt=True, use_vllm=False, verbose=True,
                   temperature=0.00001, max_new_tokens=_mnt,
                   repetition_penalty=1.0, top_p=0.8, top_k=1)
    supported_VLM['Qwen3-VL-HF'] = partial(_PP_Chat, model_path=_mp, **_common)
    supported_VLM['Qwen3.5-HF']  = partial(_PP_Chat, model_path=_mp, **_common)
# ─────────────────────────────────────────────────────────────────────────────
...
# VLMEvalKit registers the concatenated MLVU variants but not the MCQ-only
# variants. Keep these aliases local to PixelPrune so commands such as
# `--data MLVU_MCQ_64frame` do not fall through to the custom TSV loader.
supported_video_datasets.setdefault('MLVU_MCQ_8frame', partial(MLVU_MCQ, dataset='MLVU_MCQ', nframe=8))
supported_video_datasets.setdefault('MLVU_MCQ_64frame', partial(MLVU_MCQ, dataset='MLVU_MCQ', nframe=64))
supported_video_datasets.setdefault('MLVU_MCQ_1fps', partial(MLVU_MCQ, dataset='MLVU_MCQ', fps=1.0))
...
"""
    ...
    parser.add_argument('--max-samples', type=int, default=None,
                        help='Limit number of samples to process (for quick ablation)')
...
def run_local_mode(args):
    ...
    eval_id = build_eval_id() if RANK == 0 else None
    if WORLD_SIZE > 1:
        eval_id_holder = [eval_id]
    ...
    for _, model_name in enumerate(args.model):
        ...
        for _, dataset_name in enumerate(args.data):
            ...
            ds_pred_root = pred_root / dataset_name
            ds_pred_root.mkdir(parents=True, exist_ok=True)

            # Resume: copy latest checkpoint pkl from a previous interrupted run (only when --reuse)
            if args.reuse and RANK == 0:
                _ckpt_name = f'0{WORLD_SIZE}_{dataset_name}.pkl'
                _ckpt_dst = ds_pred_root / _ckpt_name
                if not _ckpt_dst.exists():
                    _prev_dirs = sorted(
                        (p for p in pred_root_meta.iterdir()
                         if p.is_dir() and p != pred_root and (p / dataset_name / _ckpt_name).exists()),
                        key=lambda p: p.stat().st_mtime, reverse=True,
                    )
                    if _prev_dirs:
                        _src = _prev_dirs[0] / dataset_name / _ckpt_name
                        shutil.copy2(_src, _ckpt_dst)
                        logger.info(f'Resumed checkpoint from {_src}')

            # Redirect PixelPrune timing logs into per-dataset subdirectory
            if os.environ.get('PIXELPRUNE_LOG_FILE', ''):
                os.environ['PIXELPRUNE_LOG_FILE'] = str(ds_pred_root / f'{dataset_name}.vit.jsonl')
            if os.environ.get('DEDUP_LOG_FILE', ''):
                os.environ['DEDUP_LOG_FILE'] = str(ds_pred_root / f'{dataset_name}.e2e.jsonl')
            ...
            try:
                result_file = get_pred_file_path(
                    str(ds_pred_root), model_name, dataset_name, use_env_format=True)
                if RANK == 0:
                ...
                if args.mode != "eval":
                    ...
                    if dataset.MODALITY == 'VIDEO':
                        if args.max_samples:
                            dataset.data = dataset.data.iloc[:args.max_samples].reset_index(drop=True)
                        model = infer_data_job_video(
                            model,
                            work_dir=ds_pred_root,
                            model_name=model_name,
                            dataset=dataset,
                            result_file=result_file,
                            verbose=args.verbose,
                ...
                if RANK == 0:
                    ...
                    files = [
                        path for path in ds_pred_root.iterdir()
                        if path.is_file() and (
