...
from bernini.device import (
    distributed_backend,
    get_inference_device,
    object_collective_device,
    set_device,
)
...
def rewrite_prompt(rewriter, task, default_task_type, ps, device):
    ...
    dist.broadcast_object_list(
        ...
        device=object_collective_device(device),
    )
    return holder[0]
...
def main():
    ...
    device = get_inference_device(local_rank)
    set_device(device)
    ...
    for task in my_tasks:
        prompt = rewrite_prompt(rewriter, task, args.task_type, ps, device)
