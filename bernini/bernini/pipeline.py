...
class BerniniPipeline:
    ...
    def __init__(
        ...
        vit_processor,
        device,
        pixelprune_config: Optional[PixelPruneConfig] = None,
    ):
        self.config = config
        self.model = model
        self.vae = vae
        ...
        self.pixelprune_config = pixelprune_config or PixelPruneConfig.from_env()
    ...
    def from_pretrained(
        ...
        **config_overrides,
        ...
        return cls(
            ...
            vit_processor,
            device,
            pixelprune_config=pixelprune_config,
        )

    def _add_profile_latency(self, name: str, latency_ms: float) -> None:
        if self.pixelprune_config.profile:
            self._profile_metrics[name] = (
                self._profile_metrics.get(name, 0.0) + float(latency_ms)
            )

    def _start_profile(self, output_path: str) -> None:
        self._profile_metrics = {
            "sample_id": os.path.splitext(os.path.basename(output_path))[0],
            "rank": dist.get_rank() if dist.is_available() and dist.is_initialized() else 0,
            "pixelprune_enabled": self.pixelprune_config.enabled,
            "pixelprune_method": self.pixelprune_config.method,
            "pixelprune_threshold": self.pixelprune_config.threshold,
            "warmup": self.pixelprune_config.warmup,
        }
        self._profile_e2e_start = time.perf_counter()
        if self.pixelprune_config.profile:
    ...
    def _finish_profile(
        ...
        output_path: str,
        *,
        e2e_excluding_io_ms: float,
        e2e_including_io_ms: float,
        write_output: bool,
    ) -> None:
        if not self.pixelprune_config.profile:
            return
        device = torch.device(self.device)
        if device.type in ("cuda", "npu"):
            synchronize(device)
            self._profile_metrics["gpu_peak_allocated_bytes"] = int(
                max_memory_allocated(device)
            )
            self._profile_metrics["gpu_peak_reserved_bytes"] = int(
                max_memory_reserved(device)
            )
        else:
            self._profile_metrics["gpu_peak_allocated_bytes"] = 0
            self._profile_metrics["gpu_peak_reserved_bytes"] = 0
        self._profile_metrics["e2e_excluding_io_ms"] = e2e_excluding_io_ms
        self._profile_metrics["e2e_including_io_ms"] = e2e_including_io_ms
        self._profile_metrics["output_written"] = write_output
        self._profile_metrics["pixelprune_overhead_ms"] = (
            self._profile_metrics.get("pixelprune_selector_latency_ms", 0.0)
            + self._profile_metrics.get(
                "pixelprune_index_conversion_latency_ms", 0.0
            )
            + self._profile_metrics.get(
                "pixelprune_sequence_rebuild_latency_ms", 0.0
            )
        )
        metrics_output = self.pixelprune_config.metrics_output
        if not metrics_output:
            metrics_output = output_path + ".metrics.jsonl"
        written_path = append_jsonl(metrics_output, self._profile_metrics)
        logger.info("PixelPrune profiling -> %s", written_path)

    def _validate_planner_branch(
        self,
        name: str,
        ...
        position_length = inputs["position_ids"].shape[-1]
        attention_shape = inputs["attention_mask_4d"].shape
        if sequence_length != position_length:
            raise AssertionError(
                f"{name}: input/position length mismatch "
                f"{sequence_length} != {position_length}"
            )
        if attention_shape[-2:] != (sequence_length, sequence_length):
            raise AssertionError(
                f"{name}: attention mask shape {attention_shape} does not match "
        ...
        for mask_name in (
            ...
            if inputs[mask_name].numel() != sequence_length:
                raise AssertionError(
                    f"{name}: {mask_name} length {inputs[mask_name].numel()} "
                    f"does not match {sequence_length}"
                )
        if self.pixelprune_config.verbose:
            logger.info(
                "PixelPrune planner branch=%s embeds=%s position=%s attention=%s "
                "visual_input=%d visual_output=%d",
                name,
                tuple(input_embeds.shape),
                tuple(inputs["position_ids"].shape),
                tuple(attention_shape),
                int(inputs["visual_input_token_mask"].sum()),
                int(inputs["visual_output_token_mask"].sum()),
            )
    ...
    def preprocess_inputs(
        ...
        def encode_visual_items(pixel_values, grid_thw, roles, item_type):
            ...
            metadata = compute_visual_keep_metadata(
                ...
                item_type=item_type,
                roles=roles,
                spatial_merge_size=mllm_model.visual.spatial_merge_size,
                config=self.pixelprune_config,
                selector_device=mllm_model.device,
            )
            if self.pixelprune_config.enabled:
                synchronize(mllm_model.device)
                self._add_profile_latency(
                    "pixelprune_selector_latency_ms",
                    (time.perf_counter() - metadata_start) * 1000,
                )
            split_for_measurement = (
                self.pixelprune_config.enabled or self.pixelprune_config.profile
            )
            if not split_for_measurement:
                embeddings = list(
                    get_vit_features(mllm_model, pixel_values, grid_thw)
                )
                pixelprune_items.extend(item.serializable() for item in metadata)
            ...
            for grid in grid_thw.tolist():
                ...
                item_pixels.append(pixel_values[offset : offset + raw_count])
                offset += raw_count

            embeddings = [None] * len(metadata)
            for role in ("source-input", "target-output"):
                indices = [index for index, value in enumerate(roles) if value == role]
                if not indices:
                    continue
                group_pixels = torch.cat([item_pixels[index] for index in indices])
                group_grid = grid_thw[indices]
                group_keep = None
                if role == "source-input" and self.pixelprune_config.enabled:
                    candidates = [
                        metadata[index].merged_keep_indices for index in indices
                    ]
                    if any(
                        metadata[index].kept_merged_token_count
                        < metadata[index].original_merged_token_count
                        for index in indices
                    ):
                        group_keep = candidates

                adapter_timings = {}
                vit_timer = CudaStageTimer(
                    self.pixelprune_config.profile, device=mllm_model.device
                )
                group_embeddings = get_vit_features(
                    mllm_model,
                    group_pixels,
                    group_grid,
                    merged_keep_indices=group_keep,
                    vit_timer=vit_timer,
                    adapter_timings=adapter_timings,
                )
                latency_key = (
                    "source_vit_latency_ms"
                    if role == "source-input"
                    else "target_vit_latency_ms"
                )
                self._add_profile_latency(latency_key, vit_timer.elapsed_ms)
                self._add_profile_latency(
                    "pixelprune_index_conversion_latency_ms",
                    adapter_timings.get("index_conversion_latency_ms", 0.0),
                )
                for index, embedding in zip(indices, group_embeddings):
                    embeddings[index] = embedding
                    metadata[index].vit_latency_ms = vit_timer.elapsed_ms
                    metadata[index].index_conversion_latency_ms = adapter_timings.get(
                        "index_conversion_latency_ms", 0.0
                    )
            pixelprune_items.extend(item.serializable() for item in metadata)
            return embeddings, metadata
        ...
        if (images is not None and len(images) > 0) or image is not None or num_frames == 1:
            ...
            image_roles = ["source-input"] * source_image_count
            image_roles.extend(["target-output"] * (len(images) - source_image_count))
            image_embeds, image_metadata = encode_visual_items(
                pixel_values, image_grid_thw, image_roles, "image"
            )
            ...
            if self.pixelprune_config.enabled:
                row_data["pixelprune_image_keep_indices"] = [
                    item.merged_keep_indices.detach().cpu().tolist()
                    if item.role == "source-input"
                    else None
            ...
            for img_tensor in image_tensors:
                vae_timer = CudaStageTimer(
                    self.pixelprune_config.profile, device=vae_model.device
                )
                with vae_timer:
                    latent = get_vae_features(vae_model, img_tensor)
                self._add_profile_latency(
                    "vae_encode_latency_ms", vae_timer.elapsed_ms
                )
        ...
        if video is not None or num_frames > 1:
            ...
            source_video_count = len(video_meta) - (1 if num_frames > 1 else 0)
            video_keep_indices = []
            for video_index, video_path in enumerate(video_meta):
                video_role = (
                    "source-input"
                    if video_index < source_video_count
                    else "target-output"
                )
                ...
                if vit_num_frames is not None:
                    if vit_num_frames <= 0:
                        raise ValueError(f"vit_num_frames must be positive, got {vit_num_frames}")
                    if vit_num_frames % 2 != 0:
                        raise ValueError(
                            f"vit_num_frames must be even for temporal_patch_size=2, got {vit_num_frames}"
                        )
                    vit_idx = torch.linspace(
                        0, video_reader.length - 1, int(vit_num_frames)
                    ).round().long().tolist()
                else:
                ...
                video_embeds, video_metadata = encode_visual_items(
                    vid_pixel_values,
                    vid_grid_thw,
                    [video_role],
                    "video",
                ...
                if self.pixelprune_config.enabled:
                    item = video_metadata[0]
                    video_keep_indices.append(
                        item.merged_keep_indices.detach().cpu().tolist()
                        if item.role == "source-input"
                        else None
                    )
                ...
                empty_cache(self.device)
            if self.pixelprune_config.enabled:
                row_data["pixelprune_video_keep_indices"] = video_keep_indices

        if pixelprune_items:
            row_data["pixelprune_items"] = pixelprune_items
    ...
    def sample_vit_embed(
        ...
        if self.model.vit_decoder is not None:
            for step in tqdm(range(planning_step), desc=f"Sample FM+MAR clip in {planning_step} steps"):
                ...
                mllm_timer = CudaStageTimer(
                    self.pixelprune_config.profile, device=device
                )
                with mllm_timer:
                ...
                self._add_profile_latency(
                    "planner_mllm_forward_latency_ms", mllm_timer.elapsed_ms
                )
                ...
                connector_timer = CudaStageTimer(
                    self.pixelprune_config.profile, device=device
                )
                with connector_timer:
                ...
                self._add_profile_latency(
                    "connector_latency_ms", connector_timer.elapsed_ms
                )
                ...
                vit_decoder_timer = CudaStageTimer(
                    self.pixelprune_config.profile, device=device
                )
                with vit_decoder_timer:
                ...
                self._add_profile_latency(
                    "vit_decoder_latency_ms", vit_decoder_timer.elapsed_ms
                )
    ...
    def __call__(
        ...
        *,
        ...
        """
        device = self.device
        self._start_profile(output_path)
        ...
        self._add_profile_latency(
            "transform_inputs_latency_ms",
            (time.perf_counter() - transform_start) * 1000,
        )
        ...
        self._add_profile_latency(
            "pixelprune_sequence_rebuild_latency_ms",
            float(
                inputs.get(
                    "pixelprune_sequence_rebuild_latency_ms",
                    torch.tensor([0.0]),
        ...
        self._validate_planner_branch("conditional", inputs_embed, inputs)
        self._validate_planner_branch(
            "unconditional", uncond_inputs_embed, uncond_inputs
        )
        self._validate_planner_branch(
            "image-conditioned", imgcond_inputs_embed, imgcond_inputs
        )
        output_query_counts = {
            int(inputs["visual_output_token_mask"].sum()),
            int(uncond_inputs["visual_output_token_mask"].sum()),
            int(imgcond_inputs["visual_output_token_mask"].sum()),
        }
        if len(output_query_counts) != 1:
            raise AssertionError(
                "PixelPrune changed target visual-output query count across planner branches"
            )

        if self.pixelprune_config.profile:
            visual_items = input_dict.get("pixelprune_items", [])
            original_visual_tokens = sum(
                item["original_merged_token_count"]
                for item in visual_items
                if item["role"] == "source-input"
            )
            kept_visual_tokens = sum(
                item["kept_merged_token_count"]
                for item in visual_items
                if item["role"] == "source-input"
            )
            self._profile_metrics.update(
                {
                    "visual_input_tokens_before": original_visual_tokens,
                    "visual_input_tokens_after": kept_visual_tokens,
                    "visual_input_token_reduction_ratio": (
                        1.0 - kept_visual_tokens / original_visual_tokens
                        if original_visual_tokens
                        else 0.0
                    ),
                    "mllm_sequence_length_after": int(inputs_embed.shape[1]),
                    "mllm_sequence_length_before": int(
                        inputs_embed.shape[1]
                        + original_visual_tokens
                        - kept_visual_tokens
                    ),
                    "target_visual_output_tokens": next(iter(output_query_counts)),
                    "visual_items": visual_items,
                }
            )

        planner_timer = CudaStageTimer(
            self.pixelprune_config.profile, device=device
        )
        with planner_timer:
        ...
        self._add_profile_latency("planner_latency_ms", planner_timer.elapsed_ms)
        ...
        empty_cache(device)
        dit_timer = CudaStageTimer(self.pixelprune_config.profile, device=device)
        with dit_timer:
        ...
        self._add_profile_latency("dit_diffusion_latency_ms", dit_timer.elapsed_ms)

        if not write_output:
            if self.pixelprune_config.profile:
                synchronize(device)
            excluding_io_ms = (time.perf_counter() - self._profile_e2e_start) * 1000
            self._finish_profile(
                output_path,
                e2e_excluding_io_ms=excluding_io_ms,
                e2e_including_io_ms=excluding_io_ms,
                write_output=False,
            )
            return None

        self.vae.to(device)
        decode_timer = CudaStageTimer(self.pixelprune_config.profile, device=device)
        with decode_timer:
            output = _vae_decode(self.vae, latents)
        self._add_profile_latency("vae_decode_latency_ms", decode_timer.elapsed_ms)
        self.vae.to("cpu")
        empty_cache(device)
        ...
        self._add_profile_latency(
            "output_write_latency_ms",
            (time.perf_counter() - output_write_start) * 1000,
        )
        including_io_ms = (time.perf_counter() - self._profile_e2e_start) * 1000
        self._finish_profile(
            output_path,
            e2e_excluding_io_ms=excluding_io_ms,
            e2e_including_io_ms=including_io_ms,
            write_output=True,
        )
