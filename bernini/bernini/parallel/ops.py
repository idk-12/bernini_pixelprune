...
def gather_seq_scatter_heads(x, seq_dim, head_dim, unpadded_dim_size=0):
    ...
    if x.size(head_dim) % ps.ulysses_size != 0:
        raise ValueError(
            f"attention heads ({x.size(head_dim)}) must be divisible by "
            f"ulysses_size ({ps.ulysses_size})"
        )
    x = _all_to_all_tensor(
        x, scatter_dim=head_dim, gather_dim=seq_dim, group=ps.ulysses_group
    )
