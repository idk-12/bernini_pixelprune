...
from ..pixelprune_utils import compress_planner_visual_inputs
...
def bernini_process_sample(
    ...
    **kwargs,
    ...
    token_num_inputs = {}
    image_keep_indices = sample.get("pixelprune_image_keep_indices")
    video_keep_indices = sample.get("pixelprune_video_keep_indices")
    ...
    for vit_type, vit_id in zip(vit_type_list, vit_img_and_vid_id_list):
        if vit_type == 0: # image
            ...
            visual_keep_indices.append(
                None if image_keep_indices is None else image_keep_indices[vit_id]
            )
        elif vit_type == 1: # video
            video_grid_thw.append(raw_video_grid_thw[vit_id])
            visual_embeds.append(raw_video_embeds[vit_id])
            visual_keep_indices.append(
                None if video_keep_indices is None else video_keep_indices[vit_id]
            )
    ...
    source_keep_indices = [
        ...
        for indices in visual_keep_indices
        if indices is not None
    ]
    tokenized_example["pixelprune_sequence_rebuild_latency_ms"] = torch.tensor(
        [0.0], dtype=torch.float64
    )
    if source_keep_indices:
        ...
        tokenized_example["pixelprune_sequence_rebuild_latency_ms"] = torch.tensor(
            [(time.perf_counter() - rebuild_start) * 1000], dtype=torch.float64
        )
