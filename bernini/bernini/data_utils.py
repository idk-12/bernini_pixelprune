...
from .device import autocast, synchronize
...
def get_vit_features(
    ...
    pixel_values,
    image_grid_thw,
    merged_keep_indices=None,
    vit_timer=None,
    adapter_timings=None,
):
    ...
    if merged_keep_indices is not None:
        merged_keep_indices = [
            indices.to(mllm_model.device) for indices in merged_keep_indices
        ]
        adapter_start = time.perf_counter()
        sparse_layout = mllm_model.visual._build_sparse_layout(
            image_grid_thw, merged_keep_indices, pixel_values.device
        )
        if pixel_values.device.type in ("cuda", "npu"):
            synchronize(pixel_values.device)
        if adapter_timings is not None:
            adapter_timings["index_conversion_latency_ms"] = (
                adapter_timings.get("index_conversion_latency_ms", 0.0)
                + (time.perf_counter() - adapter_start) * 1000
            )
    timer_context = vit_timer if vit_timer is not None else nullcontext()
    with (
        torch.no_grad(),
        autocast(mllm_model.device, dtype=torch.bfloat16),
        timer_context,
    ):
        image_embeds = mllm_model.visual(
            pixel_values,
            grid_thw=image_grid_thw,
            merged_keep_indices=merged_keep_indices,
            sparse_layout=sparse_layout,
        )
    if merged_keep_indices is None:
        split_sizes = (
            image_grid_thw.prod(-1) // mllm_model.visual.spatial_merge_size**2
        ).tolist()
    else:
        split_sizes = [int(indices.numel()) for indices in merged_keep_indices]
...
def get_vae_features(vae_model, x):
    ...
    with torch.no_grad(), autocast(vae_model.device, dtype=torch.float32):
