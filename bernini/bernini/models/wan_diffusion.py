...
def _patch_grid(transformer: WanTransformer3DModel, latent: torch.Tensor):
    """Return the DiT token grid for one spatial VAE latent tensor."""
...
class GEN_Wanx22(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.rainfusion_config = {
            "backend": getattr(config, "dit_attention_backend", "auto"),
            "sparsity": float(getattr(config, "rainfusion_sparsity", 0.7)),
            "start_step": int(getattr(config, "rainfusion_start_step", 10)),
            "block_size": int(getattr(config, "rainfusion_block_size", 128)),
        }
    ...
    def shared_step(self, model_id, noisy_latents, timesteps, cond_embeds, rotary_embs,
        ...
        return cur_transformer(
            ...
            rainfusion_segments=kwargs.get("rainfusion_segments"),
            rainfusion_step_idx=kwargs.get("rainfusion_step_idx", 0),
        ).sample
    ...
    def sample(
        ...
        for t_idx, t in enumerate(timesteps):
            ...
            target_grid = _patch_grid(cur_transformer, unpacked_noisy_latent)
            ...
            def _assemble(cond_lats, cond_rots, cond_msks, cond_len):
                return (
                    ...
                    cond_len + noisy_len,
                    {
                        "target_start": cond_len,
                        "target_length": noisy_len,
                        "target_grid": target_grid,
                    },
                )

            none_inp, none_rot, none_msk, none_total, none_segments = _assemble([], [], [], 0)
            v_inp, v_rot, v_msk, v_total, v_segments = _assemble(v_latents, v_rotary, v_masks, v_len)
            i_inp, i_rot, i_msk, i_total, i_segments = _assemble(i_latents, i_rotary, i_masks, i_len)
            vi_inp, vi_rot, vi_msk, vi_total, vi_segments = _assemble(vi_latents, vi_rotary, vi_masks, vi_len)

            timestep = t.expand(1)

            def _fwd(lat_inp, rot, msk, total, segments, text_emb):
                pred = self.shared_step(
                    ...
                    rainfusion_segments=segments,
                    rainfusion_step_idx=t_idx,
                )
                return pred[:, msk, :]
            ...
            if guidance_mode == "rv2v":
                ...
                eps_uncond = _fwd(none_inp, none_rot, none_msk, none_total, none_segments, uncond_text)
                eps_V = _fwd(v_inp, v_rot, v_msk, v_total, v_segments, uncond_text)
                eps_VI = _fwd(vi_inp, vi_rot, vi_msk, vi_total, vi_segments, uncond_text)
                eps_VTI = _fwd(vi_inp, vi_rot, vi_msk, vi_total, vi_segments, cond_text)
                noise_pred = (
                    eps_uncond
                ...
                eps_uncond = _fwd(vi_inp, vi_rot, vi_msk, vi_total, vi_segments, uncond_text)
                eps_VTI = _fwd(vi_inp, vi_rot, vi_msk, vi_total, vi_segments, cond_text)
                ...
                eps_uncond = _fwd(none_inp, none_rot, none_msk, none_total, none_segments, uncond_text)
                eps_T = _fwd(none_inp, none_rot, none_msk, none_total, none_segments, cond_text)
                ...
                eps_uncond = _fwd(none_inp, none_rot, none_msk, none_total, none_segments, uncond_text)
                eps_I = _fwd(i_inp, i_rot, i_msk, i_total, i_segments, uncond_text)
                eps_TI = _fwd(i_inp, i_rot, i_msk, i_total, i_segments, cond_text)
        ...
        **kwargs,
        ...
        for t_idx, t in enumerate(timesteps):
            ...
            noise_pred = self.sample_one_step(
                ...
                shape=shape,
                rainfusion_target_grid=target_grid,
                rainfusion_step_idx=t_idx,
            )
    ...
    def sample_one_step(
        ...
        def _seq_lens_kwargs(latent_inputs: torch.Tensor, cond_embeds: torch.Tensor):
            ...
            return dict(
                ...
                rainfusion_segments={
                    "target_start": latent_inputs.shape[1] - target_length,
                    "target_length": target_length,
                    "target_grid": rainfusion_target_grid,
                },
                rainfusion_step_idx=rainfusion_step_idx,
            )
