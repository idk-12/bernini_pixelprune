...
from ..attention import editing_rainfusion_attention, varlen_attention
...
class WanAttnProcessor2_0:
    ...
    def __call__(
        ...
        use_rainfusion = (
            not is_cross_attn
            and rainfusion_config is not None
            and rainfusion_config.get("backend") == "rainfusion"
            and rainfusion_step_idx >= rainfusion_config.get("start_step", 0)
            and rainfusion_segments is not None
        )
        if use_rainfusion:
            hidden_states = editing_rainfusion_attention(
                query, key, value,
                target_start=int(rainfusion_segments["target_start"]),
                target_length=int(rainfusion_segments["target_length"]),
                target_grid=tuple(rainfusion_segments["target_grid"]),
                sparsity=float(rainfusion_config["sparsity"]),
                block_size=int(rainfusion_config.get("block_size", 128)),
            )
        else:
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        kwargs.update(
            ...
            rainfusion_segments=rainfusion_segments,
