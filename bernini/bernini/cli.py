...
from .pixelprune_utils import PixelPruneConfig
...
def add_common_args(parser):
    ...
    g.add_argument(
        "--dit_attention_backend",
        choices=("auto", "flash", "rainfusion"),
        default="auto",
        help="DiT self-attention backend; RainFusion requires Ascend NPU inference.",
    )
    g.add_argument("--rainfusion_sparsity", type=float, default=0.7,
                   help="KV-block sparsity used by RainFusion v3 [0, 1); default: 0.7")
    g.add_argument("--rainfusion_start_step", type=int, default=10,
                   help="keep dense DiT self-attention before this diffusion step")
    ...
    g.add_argument(
        ...
        type=int,
        default=50176,
        help="max pixel budget for Qwen ViT visual preprocessing",
    )
    g.add_argument(
        "--vit_num_frames",
        type=int,
        default=None,
        help="fixed number of frames sampled for Qwen ViT video inputs",
    )
    ...
    g = parser.add_argument_group("PixelPrune (full Bernini only)")
    g.add_argument(
        "--pixelprune",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="prune source visual-input tokens before the Qwen2.5-VL ViT",
    )
    g.add_argument("--pixelprune_threshold", type=float, default=None)
    g.add_argument("--pixelprune_method", default=None)
    g.add_argument(
        "--pixelprune_verbose",
        action=argparse.BooleanOptionalAction,
        default=None,
    ...
    g.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=None,
        help="enable the shared baseline/PixelPrune profiling path",
    )
    g.add_argument("--pixelprune_metrics_output", default=None)
    g.add_argument(
        "--pixelprune_warmup",
        action=argparse.BooleanOptionalAction,
        default=None,
...
def build_pipeline(args, device):
    ...
    env_config = PixelPruneConfig.from_env()
    pixelprune_config = PixelPruneConfig(
        enabled=env_config.enabled if args.pixelprune is None else args.pixelprune,
        threshold=(
            env_config.threshold
            if args.pixelprune_threshold is None
            else args.pixelprune_threshold
        ),
        method=(
            env_config.method
            if args.pixelprune_method is None
            else args.pixelprune_method.strip().lower()
        ),
        verbose=(
            env_config.verbose
            if args.pixelprune_verbose is None
            else args.pixelprune_verbose
        ),
        profile=(
            env_config.profile
            if args.pixelprune_profile is None
            else args.pixelprune_profile
        ),
        metrics_output=(
            env_config.metrics_output
            if args.pixelprune_metrics_output is None
            else args.pixelprune_metrics_output
        ),
        warmup=(
            env_config.warmup
            if args.pixelprune_warmup is None
            else args.pixelprune_warmup
        ),
    )
    ...
    if model_type == "bernini":
        ...
        return BerniniPipeline.from_pretrained(
            ...
            dit_attention_backend=args.dit_attention_backend,
            rainfusion_sparsity=args.rainfusion_sparsity,
            rainfusion_start_step=args.rainfusion_start_step,
            pixelprune_config=pixelprune_config,
        )

    if pixelprune_config.enabled:
        raise ValueError(
            "PixelPrune only supports full Bernini (model_type='bernini'); "
            "BerniniRendererPipeline has no Qwen2.5-VL semantic planner"
        )
    if pixelprune_config.profile:
        raise ValueError(
            "--pixelprune_profile is only implemented for full Bernini; "
            "use a model_type='bernini' config"
        )
...
def generation_kwargs(args) -> dict:
    ...
    return dict(
        ...
        vit_num_frames=args.vit_num_frames,
